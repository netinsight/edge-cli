...
pub struct App {
    ...
    pub command_mode: bool,              // Vim-style command mode active
...
- `enter/exit_command_mode()` - Toggle vim-style command mode
...
- `handle_list_mode_input()` - Up/Down navigation, d/Enter (describe), Ctrl-D (delete), r (refresh), a (toggle auto-refresh), : (command mode), ? (help)
- `handle_command_mode_input()` - Text input, TAB (completion), Enter (execute), Esc (cancel)
- `handle_describe_mode_input()` - Up/Down scrolling, Esc (back to list), : (command mode)
...
├────────────────────────────────────────────┤
│ Command Input (only in command mode)      │  ← Optional
├────────────────────────────────────────────┤
│                                            │
│         Main Content Area                  │
