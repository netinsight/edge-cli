...
fn draw_top_bar(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let shortcuts = match app.view_mode {
        ViewMode::List => {
            ...
            format!(
                "<↑↓> scroll    <d> describe    {}<a> toggle refresh    <:> navigate    <?> help",
                action_shortcut
            )
        }
        ViewMode::Describe => {
...
fn draw_delete_confirm(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    if let Some(item) = app.selected_item() {
        ...
        let dialog_area = centered_rect(35, 30, area);
...
fn draw_help_view(f: &mut Frame, app: &mut App, area: Rect) {
    let help_text = vec![
        ...
        Line::from("  a             Toggle auto-refresh (enabled by default)"),
...
fn draw_about_view(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let about_text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(git_hash, Style::default().fg(Color::Yellow)),
            Span::styled(" ", Style::default()),
            Span::styled(build_date, Style::default().fg(Color::Yellow)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::raw("Copyright (c) "),
            Span::raw(year),
            Span::raw(" Net Insight AB"),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled(
